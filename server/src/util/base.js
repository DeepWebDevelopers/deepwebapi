...
class Command {
	...
	constructor(client, info) {
		...
		this.defaultHandling =
			"defaultHandling" in info ? info.defaultHandling : true;
		...
		this.argsSingleQuotes =
			"argsSingleQuotes" in info ? info.argsSingleQuotes : true;
	...
	onError(err, message, args, fromPattern, result) {
		...
		const ownerList = owners
			? owners
					.map((usr, i) => {
						const or =
							i === owners.length - 1 && owners.length > 1 ? "or " : "";
	...
	isEnabledIn(guild, bypassGroup) {
		...
		guild = this.client.guilds.resolve(guild);
		return (
			(bypassGroup || guild.isGroupEnabled(this.group)) &&
			guild.isCommandEnabled(this)
		);
	}
	...
	isUsable(message = null) {
		...
		return (
			this.isEnabledIn(message.guild) &&
			hasPermission &&
	...
	reload() {
		...
		try {
			cmdPath = this.client.registry.resolveCommandPath(
				this.groupID,
				this.memberName
			);
	...
	static validateInfo(client, info) {
		...
		if (
			...
			(!Array.isArray(info.patterns) ||
				info.patterns.some((pat) => !(pat instanceof RegExp)))
		) {
			throw new TypeError(
