...
const ArgumentUnionType = require("./union");
...
class Argument {
			...
			prompts.push(
				...
				${
					empty
						...
						: valid
						? valid
						: `You provided an invalid ${this.label}. Please try again.`
				}
				${oneLine`
	...
	validate(val, msg) {
		const valid = this.validator
			? this.validator(val, msg, this)
			: this.type.validate(val, msg, this);
		if (!valid || typeof valid === "string") return this.error || valid;
		if (valid instanceof Promise)
			return valid.then((vld) =>
				!vld || typeof vld === "string" ? this.error || vld : vld
			);
		return valid;
	}
	...
	static validateInfo(client, info) {
		...
		if (
			info.type &&
			!info.type.includes("|") &&
			!client.registry.types.has(info.type)
		) {
			throw new RangeError(`Argument type "${info.type}" isn't registered.`);
		}
		if (!info.type && !info.validate) {
			throw new Error(
				'Argument must have either "type" or "validate" specified.'
			);
		}
		...
		if (!info.type && (!info.validate || !info.parse)) {
			throw new Error(
				"Argument must have both validate and parse since it doesn't have a type."
			);
		}
		if (
			typeof info.wait !== "undefined" &&
			(typeof info.wait !== "number" || Number.isNaN(info.wait))
		) {
