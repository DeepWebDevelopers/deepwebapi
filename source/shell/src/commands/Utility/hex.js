  ...
  run: async ({ message, args, text, client, prefix, instance }) => {
    if (!args[0])
      return message.channel.send(
        "Please specify a valid codec, either decode or encode."
      );
    ...
    if (!textIn)
      return message.channel.send(
        "Please input some text, preferably in ASCII encoding. See https://theasciicode.com.ar/ for a list of available characters to choose from."
      );
