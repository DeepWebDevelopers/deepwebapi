...
module.exports = (client) => {
	client.on("guildMemberRemove", async (member) => {
		...
		const embedRemoveMember = new Discord.MessageEmbed()
			...
			.addField(
				"<:HBuser:783351289114918973> **User tag:**",
				`${member.user.tag}`
			)
			.addField(
				"<:HBinformation:783351288062672896> **User ID:**",
				`${member.user.id}`
			)
			.addField(
				"<:HBagenda:783351287847845890> **Account Create:**",
				`${member.user.createdAt.toUTCString().substr(0, 16)} (${checkDays(
