...
module.exports = {
    ...
    minArgs: 1,
    maxArgs: -1,
    expectedArgs: "<emoji (normal or custom)>",
    description: "Convery emoji to emoji**s**",
    category: "Fun & Games",
    run: async ({
        message,
        args,
        ...
        const emoji = client.emojis.cache.find(e => e.name == args[0].split(":")[1]);
        ...
        if (!emoji) {
            ...
            if (args[0].includes(".png") || args[0].includes(".jpg")) {
                ...
                if (!url.startsWith("<img")) return message.channel.send("Only works with custom emojis from this server / default emojis / png or jpg urls of emojis.");
        ...
        Jimp.read(url, (err, img) => {
            if (err) return message.channel.send("Could not read image");
            img.resize(size, Jimp.AUTO, Jimp.RESIZE_BEZIER, (err, img) => {
                if (err) return message.channel.send("Could not load image");
                const transColors = [];
