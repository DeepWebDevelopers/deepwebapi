...
module.exports = {
    ...
    run: async ({ message, args, text, client, prefix, instance }) => {
        ...
        message.channel.send(`**Starting giveaway process... Enter \`cancel\` anytime to end the process.**`)
        message.channel.send("What is the duration of the giveaway? Please enter a valid value (1d, 1m, 1h), weeks and above are not supported, if youu wish to make the duration of the giveaway a week or longer, please do something like '7d' or '10d'.")
        ...
        if (durationAnswer === 'cancel' || durationAnswer === 'Cancel') {
            message.channel.send('**Giveaway process ended, requested by command executer.**')
            return;
        }
        ...
        if (isNaN(winner)) return message.channel.send("Invalid number of winners, please restart this process.")

        await message.channel.send("Please mention the channel to post the giveaway in.")
        ...
        //Check if its cancel and cancel
        if (collector3.first().content === 'cancel' || collector3.first().content === 'Cancel') {
            message.channel.send('**Giveaway process ended, requested by command executer.**')
            return;
        }

        if (channelMention && channelMention.toString().includes("<#") && channelMention && channelMention.toString().includes(">")) {
        ...
        } else return message.channel.send("You did not mention a channel, please restart this process.")

        let channel = await client.channels.fetch(channelMention)

        if (!channel) return message.channel.send("Could not find provided channel. Please restart this process.")
